(function(){
	...
	var 纹理类 = function( 处理器, 图像, 选项, 就绪回调, 销毁方法, 名称 ){
		...
		var internalFormat = 选项.源格式 || 选项.内部格式 || gl.RGBA32F;
	...
	纹理类.prototype = {
		...
		替换图像: function( 图像, 选项, 就绪回调 ){
			...
			this._WEBGL纹理 = 载入WEBGL纹理( this._处理器.gl, 图像, _就绪回调, Object.assign(本._选项, 选项), this._WEBGL纹理 );
	...
	绘图区类.prototype = {
		...
		刷新画板: function(){
			...
			let 计数 = 0;
			let 目标集 = this._渲染目标序列.目标;
			let 序列 = [];
			for ( let i = 0; i < 画板集.length; i++ ){
				//let 记录 = 画板集[i];
				//if (记录) {
					序列.push(gl['COLOR_ATTACHMENT'+计数]);
					计数++;
					//目标集.push(记录);
				//}
			}
			...
			for (let i=0; i<目标集.length; i++){
				let 目标 = 目标集.pop();
				gl.framebufferTexture2D(gl.FRAMEBUFFER, 目标.画板编号, gl.TEXTURE_2D, 目标.WEBGL纹理, 0);
			}
console.log("刷新画板。");	
		},
		...
		添加画板: function(画板, 标识){
			...
			for ( let i = 0; i < 长度; i++ ){
				...
				if (已启用)
					...
					目标集.push({画板编号: 编号, WEBGL纹理: 画板.WEBGL纹理, 顺序: 顺序});
			...
			if ( 添加数 || 替换数 || 覆盖数 ){
				...
				//for ( let i = 0; i < 画板集.length; i++ ){
				//	let a = 画板集[i];
				//	if (a && a.画板编号) 编号.push(gl['COLOR_ATTACHMENT'+(计数++)]);
				//}
				//this._渲染目标序列.序列 = 编号;
				
				//if (已启用){
				//	if ( 编号.length ) gl.drawBuffers(编号)
				//}
				//else
					//this._渲染目标序列.目标 = 目标集;
			}
			...
			if ( 标识 === undefined ){
				...
				if ( 已找到 ){
					...
					if ( 顺序>0 ){
						...
						//	for (let i = 0; i<目标集.length; i++)
						//		gl.framebufferTexture2D(gl.FRAMEBUFFER, 序列[i], gl.TEXTURE_2D, 目标集[i].纹理.WEBGL纹理, 0);
							
						//	if ( 序列.length ) gl.drawBuffers(序列);

						//}
						//else{
							
							//for (let i = 0; i<目标集.length; i++)
							//	目标集[i] = {画板编号: 序列[i], WEBGL纹理: 目标集[i].纹理.WEBGL纹理, 顺序: 顺序};
							
							//this._渲染目标序列.目标 = this._渲染目标序列.目标.concat(目标集);
			...
			if (Object.prototype.toString.apply(标识)=="[object String]"){
				...
				if ( Object.prototype.toString.apply(序号)=="[object Number]" &&
					 ...
					 画板集[序号].纹理 instanceof 纹理类 ){
						 ...
						 const 记录 = 画板集[序号];
						 return { 纹理: 记录.纹理, 序号: 序号, 顺序: 记录.顺序 };
				...
				if ( 标识 < 画板集.length && 画板集[标识] && 
					 画板集[标识].纹理 instanceof 纹理类 ){
						...
						const 记录 = 画板集[标识];
						return { 纹理: 记录.纹理, 序号: 标识, 顺序: 记录.顺序 };
		...
		获取画板图像: function( 标识, 浮点数颜色, 转换为图像 ){
			...
			gl.readBuffer(gl['COLOR_ATTACHMENT'+记录.顺序]);
			...
			let 图像;
			if ( 转换为图像 ){
				let 数据 = new Uint8Array(  宽 * 高 * 4 );
				gl.readPixels(0, 0, 宽, 高, gl.RGBA, gl.UNSIGNED_BYTE, 数据); 
				图像 = new ImageData( new Uint8ClampedArray(数据), 宽, 高 );
			}
			else{
	...
	处理程序集.高斯模糊 = function ( 模糊半径, 内部调用 ){
		...
		const 片元着色程序源 = `#version 300 es
			...
			void main(void) {
				...
				fragColor[2] = vec4( 0.5, 0.8, 0.2, 1.0 );
				fragColor[0] = vec4( 1.0, 0.8, 0.2, 1.0 );
				...
				for ( int i = 0; i < 16384; i++ ){
					...
					for ( int j = 0; j < 16384; j++ ){
						...
						fragColor[1] += texture( uToBeProcessedImage, ( vTextureCoord - uGaosiRadius * uToBeProcessedImageInterval ) + ( gaosiCoord * uToBeProcessedImageInterval ) ) * a.a;
	...
	function 载入WEBGL纹理 (gl, 数据, 完成回调, 选项, WEBGL纹理) {
		...
		var internalFormat = 选项.内部格式 || gl.RGBA32F;
	...
	*/
	...
	function 高斯模糊( 图片, 模糊半径, 完成回调){
		...
		处理.绘制 = (完成回调)=>{
			...
			完成回调((浮点数颜色, 转换为图像)=>{return 处理器.获取绘图区('图像处理区域').获取画板图像('第二块',浮点数颜色, 转换为图像)});
		};
