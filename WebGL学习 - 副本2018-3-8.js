(function(){
	...
	var 类型化数组对应GL类型={
			"[object Float32Array]": WebGL2RenderingContext.FLOAT,
			"[object Uint16Array]": WebGL2RenderingContext.UNSIGNED_SHORT,
			"[object Int16Array]": WebGL2RenderingContext.SHORT,
			"[object Uint32Array]": WebGL2RenderingContext.UNSIGNED_INT,
			"[object Int32Array]": WebGL2RenderingContext.INT,
			"[object Int8Array]": WebGL2RenderingContext.BYTE,
			"[object Uint8Array]": WebGL2RenderingContext.UNSIGNED_BYTE,
		};
	...
	处理器类.prototype = {
		...
		let start = window.performance.now();
				...
				if (this._处理线已启动){
					...
					let 处理线 = this._处理线;
					let 处理器 = 处理线[0];
					let 返回值 = 处理器[0].apply( this, 处理器[1] );
					for (let i = 1, l = 处理线.length; i < l; i++){
						let 处理器 = 处理线[ i ];
						处理器[1][0] = 返回值[0].纹理;
						返回值 = 处理器[0].apply( this, 处理器[1] );
					}
					this.处理器返回值 = 返回值;
...
//for(let i=0;i<this._最大画板数;i++)
	//	console.log(gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, gl['COLOR_ATTACHMENT'+i], gl.FRAMEBUFFER_ATTACHMENT_OBJECT_NAME),';  刷新i:',i);
...
//console.log("刷新画板。");	
		...
		清除画板: function(标识){
			...
			if ( 标识 === undefined ){
				...
				let 画板 = new 纹理类(处理器, null, {宽: this._绘图区宽, 高: this._绘图区高, 源类型: gl.FLOAT });
		...
		const 片元着色程序源 = `#version 300 es
			...
			void main(void) {
				...
				for ( int i = 0; i < 16384; i++ ){
					...
					gaosiCoord = vec2( i , 0.0 ) + 0.1;
					b += texture( uGaosiMatrix, ( gaosiCoord ) * uGaosiInterval ).r;
					c++;
				}
				
				//fragColor[DB_default] = vec4( 0.5, 0.8, 0.2, 1.0 );
				fragColor[2] = vec4( b, uGaosiMatrixSize, c );
				fragColor[1] = texture( uGaosiMatrix, vTextureCoord )*uGaosiMatrixSize.x*0.2;
				...
				for ( int i = 0; i < 16384; i++ ){
					...
					highp vec4 a = texture( uGaosiMatrix, (gaosiCoord ) * uGaosiInterval );
					if ( uVertical ){
	...
	start = window.performance.now();
			...
			if ( (上次模糊半径 !== 模糊半径) || (上次标准差 !== 标准差) ){
				
				上次模糊半径 = 模糊半径;
				上次标准差 = 标准差;
		//模糊半径 = 10;
				var 高斯数列 = 生成正态分布数列( 模糊半径, 标准差 );
	...
	start = window.performance.now();
				...
				this.准备图像( 高斯数列, 'uGaosiMatrix', {宽: 高斯数列.length, 高: 1, 内部格式: gl.R32F, 源格式: gl.RED, 源类型: gl.FLOAT, 缩小过滤: gl.NEAREST, 放大过滤: gl.NEAREST } );
	...
	//if (gl.isTexture(图像.WEBGL纹理)) console.log('高斯模糊图像.WEBGL纹理');		
			let 宽 = 图像.宽, 高 = 图像.高;
			{
				//横向模糊。
				绘图区.更改尺寸( 宽, 高 );	
				this.添加纹理( 图像, 'uToBeProcessedImage' );			
	...
	//console.log(window.performance.now()-start);
			...
			{
				//纵向模糊。
				let 画板 = 绘图区.添加画板(new 纹理类(this, null, {宽: 宽, 高: 高, 源类型: gl.FLOAT, 多级贴图: false, 缩小过滤: gl.NEAREST, 放大过滤: gl.NEAREST}), '默认画板');
	...
	//console.log(window.performance.now()-start);
			...
			return 绘图区.添加画板(new 纹理类(this, null, {宽: this.画布宽, 高: this.画布高, 源类型: gl.FLOAT, 多级贴图: false, 缩小过滤: gl.NEAREST, 放大过滤: gl.NEAREST}), '默认画板');
	...
	处理程序集.添加程序( '查找边缘', function ( 图像, 类型, 灰度, 阈值低, 阈值高 ){
		...
		const 片元着色程序源 = `#version 300 es
			...
			highp float range[8];//pi=3.1415926535897932384626433832795;
			...
			void main(void) {
				...
				range[0] = 2.748893571891069;
				range[1] = 1.9634954084936207;
				range[2] = 1.1780972450961724;
				range[3] = 0.39269908169872414;
				range[4] = -0.39269908169872414;
				range[5] = -1.178097245096172;
				range[6] = -1.9634954084936211;
				range[7] = -2.7488935718910685;
				
				highp vec2 uToBeProcessedImageInterval = 1.0 / uToBeProcessedImageSize;

				//fragColor[DB_default] = vec4( 0.5, 0.8, 0.2, 1.0 );
				
				//texture( uToBeProcessedImage, vTextureCoord );
				...
				if ( uRestrain ){
					...
					b22 = texture( uDirectionImage, vTextureCoord );//fragColor[1];
					
					fragColor[1]=(b22+3.14151926)/(3.14151926*2.0);
					//fragColor[1]=vec4(0.0);
					//if ( b22.r <= range[7] || b22.r >= range[0] ) fragColor[1]=vec4(1.0);
					fragColor[1].a=1.0;
					fragColor[2] = b22;//vec4( uRestrain, uDubbleThreshold, fragColor[2].w+5.0 );
					...
					//fragColor[2] = vec4( uRestrain, uDubbleThreshold, fragColor[2].w+5.0 );
				}
				else{
					// Scharr算子
					...
					if ( uGray ){
						...
						//fragColor[1] = vec4( atan(1.0, 1.0 ), atan(1.0, -1.0 ), atan(-1.0, -1.0 ), atan(-1.0, 1.0 ));
					}
					else{
		...
		return function ( 图像, 类型, 灰度, 阈值低, 阈值高 ){
			...
			let 宽 = 图像.宽, 高 = 图像.高;
			let canny = false;
			let 选项 = {宽: 宽, 高: 高, 源类型: gl.FLOAT, 多级贴图: false, 缩小过滤: gl.NEAREST, 放大过滤: gl.NEAREST};
			...
			if ( canny ){
				...
				let 画板 = 绘图区.添加画板([new 纹理类(this, null, 选项), new 纹理类(this, null, 选项)], ['默认画板', 1]);
				this.添加纹理( 画板[0].纹理, 'uToBeProcessedImage' );
				程序信息.绑定纹理( 'uToBeProcessedImage' );
		...
		//return this.查找边缘( 图像, 类型, 阈值低, 阈值高 );
	...
	*/
	...
	function 匹配WebGL内部格式与源格式(像素格式或数据类型){
		{
			...
			for(let i=0;i<格式组合数组.length;i++){
				...
				if (格式组合.length === 3){
					let 格式 = 格式组合[0];
					if(格式) internalFormat=WebGL2RenderingContext[格式];
					格式 = 格式组合[1];
					if(格式) format=WebGL2RenderingContext[格式];
					格式 = 格式组合[2];
					if(格式) type=WebGL2RenderingContext[格式];
	...
	function 获取WebGL常量名(常量){
		var WebGL常量 = {};
		var keys = Object.keys(WebGL2RenderingContext);
		for (let i = 0, l = keys.length; i < l; i++){
			let 常量 = WebGL2RenderingContext[ keys[i] ];
	...
	function 高斯模糊( 图片, 模糊半径, 完成回调){
		...
		处理器.设置处理线([ 
							[ 处理程序集.获取程序('高斯模糊'),[ 'uToBeProcessedImage', 2 ] ], 
							//[ 处理程序集.获取程序('查找边缘'),[ 'uToBeProcessedImage', 'Sobel' ] ],
							[ 处理程序集.获取程序('查找边缘'),[ 'uToBeProcessedImage', 'Canny', false, 0.25, 0.5 ] ],//Canny
							//[ 处理程序集.获取程序('高斯模糊'),[ 'uToBeProcessedImage', 模糊半径 ] ], 
							//[ 处理程序集.获取程序('查找边缘'),[ 'uToBeProcessedImage', 模糊半径 ] ],
						]);
						
		//处理器.设置处理程序( 处理程序集.获取程序('高斯模糊'), [ 'uToBeProcessedImage', 模糊半径 ] );
		
		//处理器.设置处理程序( 处理程序集.获取程序('查找边缘'), [ 'uToBeProcessedImage', 模糊半径, 5 ] );
		...
		处理器.设置完成回调(完成回调, [
			(标识, 浮点数颜色, 转换为图像)=>{
				if (处理器.处理器返回值 && 处理器.处理器返回值[0] && (处理器.处理器返回值[0].纹理 instanceof 纹理类)){
					处理器.获取绘图区('图像处理区域').添加画板(处理器.处理器返回值[0].纹理, '默认画板');
		...
		处理.绘制 = (完成回调)=>{
			...
			完成回调((标识, 浮点数颜色, 转换为图像)=>{return 处理器.获取绘图区('图像处理区域').获取画板图像(标识, 浮点数颜色, 转换为图像)});
		};
