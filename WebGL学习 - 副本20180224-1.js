(function(){
	...
	处理器类.prototype = {
		...
		获取纹理: function(标识){
			...
			if (Object.prototype.toString.apply(标识)=="[object String]"){
				...
				if ( Object.prototype.toString.apply(序号)=="[object Number]" &&
					 ...
					 纹理集[序号].纹理 instanceof WebGLTexture ){
						 
						 const 记录 = 纹理集[序号];
						 return { 纹理: 记录.纹理, 序号: 序号, 宽: 记录.宽, 高: 记录.高 };
				...
				if ( 标识 < 纹理集.length && 纹理集[标识] && 
					 纹理集[标识].纹理 instanceof WebGLTexture ){
						
						const 记录 = 纹理集[标识];
						return { 纹理: 记录.纹理, 序号: 标识, 宽: 记录.宽, 高: 记录.高 };
		...
		添加绘图区: function(标识, 画板, 选项){
			...
			if ( 绘图区 instanceof WebGLFramebuffer ){
				console.warn('绘图区“'+标识+'”已存在。');
				return;
			}
			else{
				
				绘图区 = new 绘图区类(this, 画板); 
				绘图区集[标识] = {标识: 绘图区 };
	...
	var 纹理类 = function( 处理器, 图像, 选项, 就绪回调, 销毁方法 ){
		...
		this._纹理 = 载入纹理( gl, 图像, this._就绪回调, 选项 );
		
		this._销毁方法 = 销毁方法;
		
		this._默认销毁方法 = function(){
			
			gl.deleteTexture( this._纹理 );
			this._纹理 = null;
	...
	纹理类.prototype = {
		...
		替换图像: function( 图像, 选项, 就绪回调 ){
			...
			this._纹理 = 载入纹理( this._处理器.gl, 图像, _就绪回调, 本._选项 );
	...
	绘图区类.prototype = {
		...
		添加画板: function(画板, 标识){
			...
			for ( let i = 0; i < 长度; i++ ){
				...
				if (Object.prototype.toString.apply(画板)!="[object Object]" && !( 画板.纹理 instanceof  WebGLTexture )){
					console.warn("“画板”参数无效！");
					break;
				}
				...
				if ( !( Object.prototype.toString.apply(序号)=="[object Number]" &&
					 ...
					 画板集[序号].纹理 instanceof WebGLTexture ) ){
				...
				gl.framebufferTexture2D(gl.FRAMEBUFFER, ca, gl.TEXTURE_2D, 画板.纹理, 0);
		...
		获取画板: function(标识){
			...
			if (Object.prototype.toString.apply(标识)=="[object String]"){
				...
				if ( 标识 < 画板集.length && 画板集[标识] && 
					 画板集[标识].纹理 instanceof WebGLTexture ){
	...
	var 着色程序信息类 = function(处理器, 顶点着色程序源, 片元着色程序源){
		if ( !( 处理器 instanceof 处理器类 ) ){
			throw "“处理器”参数无效！";
			return;
		}
	...
	着色程序信息类.prototype = {
		...
		*/
		...
		绑定纹理:  function( 纹理变量名 ){
			...
			this.设置本地一致变量( 纹理变量名 + 'Size', [记录.宽, 记录.高], '2f' );

			gl.activeTexture( gl['TEXTURE' + 记录.序号] );
			gl.bindTexture(gl.TEXTURE_2D, 记录.纹理);
	...
	var 图像处理着色程序信息类 = function(处理器, 片元着色程序源){
	...
	图像处理着色程序信息类.prototype = Object.assign( Object.create( 着色程序信息类.prototype ), 
		{
			...
			设置程序: function( 片元着色程序源 ){
				...
				着色程序信息类.prototype.设置程序.call( this, this._顶点着色程序源, 片元着色程序源 );
			...
			void main(void) {
				...
				for ( int i = 0; i < 16384; i++ ){
					...
					for ( int j = 0; j < 16384; j++ ){
						...
						gl_FragColor += texture2D( uToBeProcessedImage, ( vTextureCoord - uGaosiRadius * uToBeProcessedImageInterval ) + ( gaosiCoord * uToBeProcessedImageInterval ) ) * a.a;
	...
	let start = window.performance.now();
		...
		模糊半径=10;
			...
			/*	本地一致变量: {
					图像长宽: 获取本地一致变量(gl, shaderProgram, 'uImageSize'),
					图像: 获取本地一致变量(gl, shaderProgram, 'uSampler'),
					高斯矩阵尺寸: 获取本地一致变量(gl, shaderProgram, 'uGaosiMatrixSize'),
					高斯矩阵: 获取本地一致变量(gl, shaderProgram, 'uGaosiMatrix'),
				},
				
				一致变量缓冲区:{
					图像长宽: { 缓冲区: new Uint32Array([宽, 高]),
								类型: '2ui',
								大小: 2,
							},								
					...
					高斯矩阵尺寸: { 缓冲区: new Uint32Array([高斯矩阵.边长, 高斯矩阵.边长]),
								类型: '2ui',
								大小: 2,
							},				
					
					高斯矩阵: { 缓冲区: 载入纹理(gl, 高斯矩阵.矩阵, null, {
									多级贴图: false, 
									缩小过滤: gl.NEAREST, 
									放大过滤: gl.NEAREST,
									源类型: gl.FLOAT,
			...
			const buffers = 初始化缓冲区(gl);

			绘制场景(gl, programInfo);
			
			处理.绘制=()=>{绘制场景(gl, programInfo);}//测试
			*/
		} 
	...
	// Draw the scene.
	//
	/*function 绘制场景(gl, programInfo, buffers, 纹理) {
		...
		//gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
		  ...
		  if (programInfo.attribLocations.position!==null){
		  ...
		  if (programInfo.attribLocations.vertexColor!==null){
	...
	*/
	...
	function 高斯模糊( 图片, 模糊半径, canvas ){
		
		var 处理器 = new 处理器类(canvas);
