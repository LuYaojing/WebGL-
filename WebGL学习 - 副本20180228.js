	...
	绘图区类.prototype = {
		...
		启用: function () {
			...
			if (目标集.length) {
				...
				let 序列 = this._渲染目标序列.序列;
				if (序列.length) ext.drawBuffersWEBGL(序列);
		...
		添加画板: function (画板, 标识) {
			...
			for (let i = 0; i < 长度; i++) {
				...
				let 编号 = ext['COLOR_ATTACHMENT' + 顺序 + '_WEBGL'];
				画板集[序号].画板编号 = 编号;

				if (已启用)
					gl.framebufferTexture2D(gl.FRAMEBUFFER, 编号, gl.TEXTURE_2D, 画板.WEBGL纹理, 0)
				else
					目标集.push({ 画板编号: 编号, WEBGL纹理: 画板.WEBGL纹理 });
			...
			if (添加数 || 替换数 || 覆盖数) {
				...
				for (let i = 0; i < 画板集.length; i++) {
					let a = 画板集[i];
					if (a && a.画板编号) 编号.push(a.画板编号);
				}
				this._渲染目标序列.序列 = 编号;

				if (已启用) {
					if (编号.length) ext.drawBuffersWEBGL(编号)
				}
				else
					this._渲染目标序列.目标 = 目标集;
			}
			...
			if (标识 === undefined) {
				...
				if (已找到) {
					...
					if (顺序 > 0) {
						...
						for (let i = 0; i < 目标集.length; i++) 序列.push(ext['COLOR_ATTACHMENT' + 目标集[i].顺序 + '_WEBGL']);
						...
						if (已启用) {
							...
							for (let i = 0; i < 目标集.length; i++)
								目标集[i] = { 画板编号: 序列[i], WEBGL纹理: 目标集[i].纹理.WEBGL纹理 };
		...
		获取画板图像: function (标识, 浮点数颜色, 转换为图像) {
			//if (处理器._读取绘图区 === this) return null;
			const 处理器 = this._处理器;
			const 读取绘图区 = 处理器._读取绘图区;
			...
			if (this._画板数 > 1) {
				读取绘图区.更改尺寸(this.宽, this.高);
				读取绘图区.添加画板(记录.纹理, '默认画板');
				return 读取绘图区.获取画板图像('默认画板', 浮点数颜色, 转换为图像);
			}
	...
	处理程序集.高斯模糊 = function (模糊半径, 内部调用) {
		...
		const 片元着色程序源 = `
			...
			void main(void) {
				...
				gl_FragData[2] = vec4( 0.5, 0.8, 0.2, 1.0 );
				...
				for ( int i = 0; i < 16384; i++ ){
					...
					for ( int j = 0; j < 16384; j++ ){
						...
						gl_FragData[1] += texture2D( uToBeProcessedImage, ( vTextureCoord - uGaosiRadius * uToBeProcessedImageInterval ) + ( gaosiCoord * uToBeProcessedImageInterval ) ) * a.a;
